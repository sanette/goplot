...
let golist filename =
  ...
  then Printf.sprintf
      "Warning: file has version %s while running gOplot has version %s. This \
       may cause errors." version Init.version
       |> print_endline;
  let rec loop golist = 
    let keyword = try
    ...
    if keyword = "end" then golist
           ...
           (match keyword with
            | "axis" ->
              Scanf.bscanf buffer "{ x = %s@; y = %s@}\n" 
                (fun sx0 sy0 -> Axis ({ x=sx0; y=sy0 }, dumb ()))
            | "func" ->
              Scanf.bscanf buffer
                "{ formula = %s@; var = %s@} , { min = %s@; max = %s@}\n"
                (fun sf x sx0 sx1 -> Func ({ formula=sf; var=String.trim x}, { min=sx0; max=sx1 }, dumb ()) )
            | "param" ->
              Scanf.bscanf buffer
                "{ formula = %s@; var = %s@} , { formula = %s@; var = %s@} , { min = %s@; max = %s@}\n"
                (fun sfx sx sfy sy ss0 ss1 ->
                   Param ({ formula=sfx; var=String.trim sx}, { formula=sfy; var=String.trim sy}, 
                          { min=ss0; max=ss1 }, dumb ()))
            | "text" ->
              Scanf.bscanf buffer "{ msg =%s@\n format = %s@; pos = { x = %s@; y = %s@}; size = %d ; alignment = %s@}\n"
            ...
            | "color" ->
              Scanf.bscanf buffer "{r=%f; g=%f; b=%f }\n"
                (fun r g b -> Color { Plt.r=r; Plt.g=g; Plt.b=b })
            | "anim" ->
              Scanf.bscanf buffer "{formula2=%s@; var1=%s@; var2=%s@},\n{min=%s@; max=%s@},\n{min=%s@; max=%s@}\n"
                (fun sf x t sx0 sx1 st0 st1 ->
                   Anim_func ({ formula2=sf; var1=x; var2=t },
                              { min=sx0; max=sx1 }, { min=st0; max=st1 }))
            | "surf3d" ->
            ...
            | "grid" ->
              Scanf.bscanf buffer "{formula2=%s@; var1=%s@; var2=%s@},\n{min=%s@; max=%s@},\n{min=%s@; max=%s@}\n"
                (fun sf x y sx0 sx1 sy0 sy1 ->
                   Grid ({ formula2=sf; var1=x; var2=y }, 
                         { min=sx0; max=sx1 }, { min=sy0; max=sy1 }, dumb ()))
            | "pause" ->
              Scanf.bscanf buffer "%d\n"
                (fun t -> Pause (Soft, t))
            | "freeze" ->
              Scanf.bscanf buffer "%d\n"
                (fun t -> Pause (Freeze, t))
            | "rotate" ->
              Scanf.bscanf buffer "{x3=%s@; y3=%s@; z3=%s@},\n%s@,\n{min=%s@; max=%s@}\n"
                (fun sx sy sz sa st0 st1 ->
                   Motion {move=Rotate ({x3=sx; y3=sy; z3=sz}, sa);
