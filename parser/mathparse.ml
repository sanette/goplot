...
let rec printer e =
 ...
 (match o with
    ...
    | Minus  -> "-" 
    | Times -> "*") in
 sprintf "%s (%s) %s" (printer e1) opname (printer e2)
    | Finop (Div,e1,e2) -> sprintf "%s (/) %s" 
 (printer e1) (printer e2)
    | Finop (Power,e1,e2) -> sprintf "%s (^) %s" 
 (printer e1) (printer e2)
    | Fun (f,e) -> sprintf "%s(%s)" f (printer e)
    | Var x -> x
    | Uminus e  -> sprintf "-%s" (printer e)
...
let rec typeur e =
  ...
  | Int i -> I (IInt i)
  | Float f -> F (FFloat f)
  | Minop (o, e1, e2) -> begin match (typeur e1, typeur e2) with
  ...
  | Finop (o, e1, e2) -> begin match (typeur e1, typeur e2) with 
  ...
  | Fun (f,e) -> begin match (typeur e) with
      | I ie -> begin
          try
  ...
  | Uminus e -> begin match (typeur e) with
      | (I ie) -> I (IUminus ie)
      | (F fe) -> F (FUminus fe)
    end
...
let tprinter etypee =
  let rec loop et profondeur =
    ...
    let binprint e1 op e2 =
      ...
      if profondeur = 0 then s else sprintf "(%s)" s 
    in
    match et with
    | I ie -> begin match ie with
        | IInt i -> if i >=0
          then string_of_int i
          else sprintf "(%i)" i
        | IMinop (o, ie1, ie2) ->
          let opname = (match o with
              | Plus -> "+"
              | Minus -> "-" 
              | Times -> "*") in
          binprint (I ie1) opname (I ie2)
        | IIFun (f, ie1) -> sprintf "%s(%s)" f (loop (I ie1) p)
        | IFFun (f, fe1) -> sprintf "%s(%s)" f (loop (F fe1) p)
        | IUminus ie1 -> sprintf "(-%s)" (loop (I ie1) p)
      end
    | F fe -> begin match fe with
        | FIntConversion ie -> sprintf "float %s" (loop (I ie) p)
        ...
        | FFFun (f,fe1) -> sprintf "%s(%s)" f (loop (F fe1) p)
        | FIFun (f,ie1) -> sprintf "%s(%s)" f (loop (I ie1) p)
        | FVar x -> x
        | FUminus fe1 -> sprintf "-.%s"(loop (F fe1) p)
      end in
  loop etypee 0
...
let string2name_table =
  create_hashtable 64 [
    ...
    ("round", FIocaml (fun x -> int_of_float (floor (x +. 0.5))));
    ("Mathutils.pos", FFocaml Mathutils.pos);
    ("Mathutils.sign", FFocaml Mathutils.sign);
...
let ocamlname s = 
  ...
  with Not_found ->
    raise (Invalid_argument
             (sprintf "Unknown keyword '%s' found when parsing." s))
